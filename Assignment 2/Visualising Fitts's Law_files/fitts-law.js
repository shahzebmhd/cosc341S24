...
var fittsTest = {
	...
	exportToCSV: function() {
        const csvRows = [];
        const headers = ["time", "distance", "width", "hit", "start_x", "start_y", "target_x", "target_y", "target_width", "path"];
        csvRows.push(headers.join(','));

        this.data[this.currentDataSet].data.forEach(dataPoint => {
            const values = [
                dataPoint.time,
                dataPoint.distance,
                dataPoint.width,
                ...
                dataPoint.target.y,
                dataPoint.target.w,
                JSON.stringify(dataPoint.path) // Convert the path to a JSON string
            ];
        ...
        a.setAttribute('download', 'fitts_law_data.csv');
...
function setupTargets(radius, width) {
    ...
    angles.forEach((angle, index) => {
        const x = centerX + radius * Math.cos(angle) * 1000; // converting to pixels
        const y = centerY + radius * Math.sin(angle) * 1000; // converting to pixels
        const target = document.createElement('div');
        target.classList.add('target');
        target.style.width = `${width * 1000 * 2}px`; // converting to pixels
        target.style.height = `${width * 1000 * 2}px`; // converting to pixels
